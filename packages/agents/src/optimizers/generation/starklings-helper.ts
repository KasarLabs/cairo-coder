 ...
 */
export async function ensureStarklingsRepo(targetPath: string): Promise<boolean> {
  ...
  try {
    ...
    console.log('Checking out the specific branch mentioned in requirements...');
    execSync(
      'git checkout feat/upgrade-cairo-and-use-scarb',
      { cwd: targetPath, stdio: 'pipe' }
    );

    console.log('Current branch:', execSync('git branch --show-current', { cwd: targetPath }).toString().trim());
