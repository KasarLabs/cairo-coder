...
export class TokenTracker {
  ...
  public static trackFullUsage(
    ...
    // Check if resultObj is directly an AIMessage (like from Gemini)
    if (
      resultObj &&
      typeof resultObj === 'object' &&
      'content' in resultObj &&
      resultObj._getType &&
      resultObj._getType() === 'ai'
    ) {
      // Try standard `usage_metadata` first
      if (resultObj.usage_metadata) {
        let {
          input_tokens = 0,
          output_tokens = 0,
          total_tokens = 0,
      ...
      if (resultObj.response_metadata) {
        // OpenAI format
        if ('tokenUsage' in resultObj.response_metadata) {
          let {
            promptTokens = 0,
            completionTokens = 0,
            totalTokens = 0,
