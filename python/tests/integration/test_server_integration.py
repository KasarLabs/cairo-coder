...
from cairo_coder.server.app import create_app, get_vector_store
...
class TestServerIntegration:
    ...
    def mock_vector_store(self):
        ...
        mock_store.similarity_search = AsyncMock(return_value=[
            Document(
                ...
                metadata={"source": "cairo-book", "page": 1}
            ),
            Document(
                page_content="Smart contracts in Cairo are deployed on Starknet",
                metadata={"source": "starknet-docs", "page": 5}
            )
        ])
        return mock_store
    ...
    @pytest.fixture
    def app(self, mock_vector_store, mock_config_manager):
...
class TestGetVectorStore:
    """Test the get_vector_store dependency function."""

    def test_get_vector_store_configuration(self):
        """Test that get_vector_store creates proper configuration."""
        with patch('cairo_coder.server.app.VectorStore') as mock_vector_store_class:
            mock_instance = Mock()
            mock_vector_store_class.return_value = mock_instance
            
            result = get_vector_store()
            
            # Check that VectorStore was called with config
            mock_vector_store_class.assert_called_once()
            call_args = mock_vector_store_class.call_args[0][0]
            
            # Check config structure
            assert hasattr(call_args, 'host')
            assert hasattr(call_args, 'port')
            assert hasattr(call_args, 'database')
            assert hasattr(call_args, 'user')
...
class TestServerStartup:
    ...
    def test_server_startup_with_mocked_dependencies(self):
    ...
    def test_server_main_function_configuration(self):
        ...
        from cairo_coder.server.app import create_app, get_vector_store, CairoCoderServer, TokenTracker
