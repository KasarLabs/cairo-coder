...
def _(dspy, retrieval_program):
    ...
    response = retrieval_program(
        query="Write a simple Cairo contract that implements a counter. Make it safe with Openzeppelin"
    )
...
def _(dspy):
    ...
    data = [
        dspy.Example(**d, chat_history="").with_inputs("query", "chat_history")
...
def _(dspy):
    ...
    class RetrievalRecallPrecision(dspy.Signature):
        ...
        recall: float = dspy.OutputField(
            desc="fraction (out of 1.0) of expected output covered by the system response"
        )
        precision: float = dspy.OutputField(
            desc="fraction (out of 1.0) of system response covered by the expected output"
        )
    ...
    class DecompositionalRetrievalRecallPrecision(dspy.Signature):
        ...
        system_search_queries: list[str] = dspy.InputField()
        system_resources: list[str] = dspy.InputField()
        expected_key_ideas: str = dspy.OutputField(
            desc="enumeration of key ideas in the expected search queries and resources"
        )
        system_key_ideas: str = dspy.OutputField(
            desc="enumeration of key ideas in the system search queries and resources"
        )
        discussion: str = dspy.OutputField(
            desc="discussion of the overlap between expected and system output"
        )
        recall: float = dspy.OutputField(
            desc="fraction (out of 1.0) of expected output covered by the system response"
        )
